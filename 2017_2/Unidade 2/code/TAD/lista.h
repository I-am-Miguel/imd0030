...
public:
	...
	T getValor();
	weak_ptr<Node<T>> getNext();
	void setNext(weak_ptr<Node<T>> nextPtr);
...
template <typename T>
weak_ptr<Node<T>> Node<T>::getNext() {
	return proximo;
}

template <typename T>
void Node<T>::setNext(weak_ptr<Node<T>> nextPtr) {
	proximo = nextPtr;
}
